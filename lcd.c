...
{
    lcd_e_high();
	i2c_write_lcd(pcf8574_ports); // Daten an PCF8574 senden
    lcd_e_delay();
    lcd_e_low();
	i2c_write_lcd(pcf8574_ports); // Daten an PCF8574 senden
}
...
static void lcd_write(uint8_t data,uint8_t rs) 
{
    ...
    {
        // configure data pins as output
        pcf8574_ports &= ~( (1<<LCD_DATA0_PIN) | (1<<LCD_DATA1_PIN) | (1<<LCD_DATA2_PIN) | (1<<LCD_DATA3_PIN));
        i2c_write_lcd(pcf8574_ports); // Daten an PCF8574 senden
...
void lcd_init(uint8_t dispAttr)
{
	...
	*/
	...
	i2c_write_lcd(0xFF); // Alles auf high
